# native-output — 한국어 출력 기준 (Claude Code)

## 개요

- 정의: AI 가 쓰는 한국어에 잘 쓴 글의 기준을 적용하는 Claude Code 스킬이다.
  - 목표는 잘 쓴 글이고, AI 티가 사라지는 것은 그 결과다.
- 적용: 설치하면 한국어가 포함된 산출물 전부에 적용된다. 훅이 결정론으로 강제한다.
- 환경: Claude Code 용이다. Claude 앱용은 outputs 가 따로 있다.

## 판정 방식

- 글 종류 판정: AI 상투어를 일괄 제거하지 않는다.
  - "결론적으로" 는 보고서에서 정당한 어휘이고 회고 글에서 AI 티다.
  - 글 종류별 판정표는 `standards/genres.md` 가 갖는다.

- 실측 수치: 글 종류별 리듬이 잘 쓴 글을 측정한 값으로 들어 있다.
  - 문장 길이 중앙값이 캐주얼 단문 26.5자, 보고서 80자다.

- 글 종류 분리: 산문 규칙을 산문이 아닌 곳에 옮기지 않는다.
  - 슬라이드 제목은 압축 명사구이고, 화면 라벨은 문장이 아니라 이름이다.
...
- `standards/`: 글쓰기 기준. 판단·문장·글·표현·글 종류의 다섯 레이어와 전용 레이어 셋.
- `standards/genres/`: 종류별 팩 7종. 실측 수치 포함.
